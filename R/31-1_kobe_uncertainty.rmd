...
create_generic_kobe_plot <- function(data, 
                                     ...
                                     custom_point = NULL,   # list(x = , y = , color = "blue", shape = 19, size = 3)
  ...
  p <- Reduce(`+`, c(
    ...
    list(
      ...
      scale_y_continuous(limits = ylim_val, expand = c(0,0),
                         sec.axis = if (!is.null(scenario_label)) sec_axis(~ ., name = scenario_label) else waiver()),
      labs(x = if (show_xlab) x_label else NULL, y = if (show_ylab) y_label else NULL),
      theme(
...
```{r steepness, echo=FALSE, message=F, fig.dim=c(10,10)}
