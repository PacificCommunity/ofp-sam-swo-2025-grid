...
knitr::opts_chunk$set(dev = "png",
                      ...
                      warning = F,
                      message = F, 
                      fig.path='../../report/Figures/Kobe_Majuro/'
                      )
...
#source("../../../R/30_estimation_uncertainty.R")
...
```{r majuro, echo=FALSE, message=F, fig.dim=c(8,6)}


load("../../../model_list/ensemble.RData")

ensemble<-ensemble$refpts %>% as.data.frame()


diag<-ensemble %>% filter(Model== "N_h08_Rb_Vb_Dbas_Mest") 




majuro<- create_generic_kobe_plot(as.data.frame(ensemble),
                                    y_var="Frecent_Fmsy",
                                    x_var="SBrecent_SBF0", 
                                    y_ref = 1,
                                    x_ref= 0.2,
                                    xlim_val = c(0, 1),
                                    quadrant_colors = c("#51C87A", "#DC143C", "darkorange", "#DC143C"),
                                     show_summary = "median",  show_hdr = F,
                                    show_points = TRUE,
                                    point_alpha = 0.3, 
                                    x_label = bquote(SB[recent]/SB["F=0"]),
                                    y_label = bquote(F[recent]/F[MSY]),
                                    custom_point = list(y = diag$Frecent_Fmsy,
                                                        x= diag$SBrecent_SBF0,
                                                        shape = 19, 
                                                        color = "blue", 
                                                        size = 2.5), 
                                    point_size = 2.5,
...
```{r majuro_ts, echo=FALSE, message=F, fig.dim=c(8, 6)}


majuro_ts=read.csv(file="../../../model_list/majuro.csv")
...
create_generic_kobe_plot(
  ...
  arrow_angle = 40, 
  y_label = bquote(F[t]/F["MSY,t"]),
  x_label = bquote(SB[t]/SB["F=0,t"]),
  
  scenario_label ="Majuro (time-dynamic)"
)
